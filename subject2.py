...
def initial_setting():
    # 파일 읽기 및 정리
    file = open('/Users/colin/llmProject/pythonProject1/resources/project_data_카카오싱크.txt', 'r',
                encoding='utf-8')
    contents = file.read()
...
def llm_initial_setting(query):
    ...
    doc_path = str('/Users/colin/llmProject/pythonProject1/resources/project_data_카카오싱크.txt')
    ...
    #agent call전에 데이터를 적정사이즈로 잘라주는 과정을 거쳐야 응답에 토큰이 부족하지 않다.
    #토큰정보를 전역변수로 선언해서 지나갈때마다 토큰수를 관리해야한다.
